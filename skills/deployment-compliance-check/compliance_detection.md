...
Extract `claims.appid` from Activity Logs. In KQL, normalize `Claims` before parsing: `parse_json(tostring(Claims))`.
...
### 2.5 Detect bootstrap-only state

Classify as **NON-COMPLIANT BOOTSTRAP** when all of the following are true:

- The only active revision runs the public bootstrap image.
- Deployment tags use placeholders such as `commit-sha=initial` and `pipeline-run-id=initial`.
- The configured ACR has no application image repository.
- No prior revision exists to reactivate.

This is not a revert candidate. Its remediation target is an approved CI/CD deployment of a pipeline-labeled image to the configured ACR.
