...
The approved GitHub Actions client ID is rendered by `scripts/post-deploy.sh` as `__APPROVED_PIPELINE_APP_ID__`. Known Portal, CLI, PowerShell, Visual Studio, or Azure Mobile App IDs are NON-COMPLIANT. A caller containing `@` is a user principal and is NON-COMPLIANT. Only an exact `claims.appid` match to the rendered value proceeds to image verification. An unknown service principal, or an unconfigured client ID, remains INVESTIGATE unless image evidence proves non-compliance. Caller identity takes precedence over tags.
