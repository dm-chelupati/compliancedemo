...
description: >
  Checks whether Azure Container App deployments comply with the organization's
  CI/CD-only deployment policy. Uses three signals: Activity Log caller identity,
  Docker image labels (tamper-proof), and resource tags.
tools:
...
See [compliance_detection.md](compliance_detection.md) for the detailed decision tree and well-known app IDs.
...
To catch this:
1. Get the currently running image tag from the Container App
2. Retrieve the image config from ACR and check for the expected labels (deployed-by=pipeline, commit-sha, pipeline-run-id, etc.)
3. If labels are missing or invalid → **NON-COMPLIANT** regardless of caller
